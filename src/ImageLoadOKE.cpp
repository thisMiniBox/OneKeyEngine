...
const VEC::Size &Picture::image_size() const
{
    return m_imageSize;
}
...
int Picture::channels() const
{
    return m_channels;
}

void Picture::set_draw_rectangle(const VEC::RectangleF &rect)
{
    transform.position=VEC::Vec3(rect.left,rect.top);
}

VEC::Color Picture::get_color(int x, int y)const
{
    VEC::Color color;
...
unsigned char *Picture::get_pixel_data(int x, int y)const
{
    if(!m_drawData)return nullptr;
    return m_drawData->data+((y*m_imageSize.width+x)*m_bitDepth*m_channels*m_bitDepth);
}
void Picture::set_pixel(int x, int y, const VEC::Color &color)
{
    delete_win32_bitmap();
    auto pixel=get_pixel_data(x,y);
    if(pixel!=nullptr)
...
ImageLoadResult Picture::load_image_form_file(const std::string &filePath)
{
...
ImageLoadResult Picture::load_image(unsigned char *imageData,int len)
{
    if(!imageData)
...
ImageLoadResult Picture::set_image(unsigned char *bitmapData, int width, int height, int channel, int bit)
{
    if (!bitmapData)
...
unsigned char *Picture::get_data() const
{
    if(m_drawData)
...
ImageLoadResult Picture::create_image(int width, int height, VEC::Color defaultFill, int channels)
{
...
std::vector<u_char> Picture::flip(bool x, bool y) const
{
...
std::vector<u_char> Picture::get_8bit_image_data() const
{
    if(!m_drawData)
...
WindowDrawResult Picture::draw(WindowDefine::WindowBase *window)
{
...
#ifdef _WIN32
HBITMAP Picture::get_win32_image()
{
    if(!m_drawData)
    {
        return nullptr;
...
void Picture::delete_win32_bitmap()
{
    if(m_drawData)
...
#endif
#ifdef __glad_h_
uint32_t Picture::get_openGL_id()
{
    if (!m_drawData)
        return 0;
...
uint32_t Picture::get_openGL_id() const
{
    if(m_drawData)
        return m_drawData->openGLID;
    return 0;
}
void Picture::delete_openGL_buffer()
{
    if(m_drawData)
...
WindowDrawResult WindowGL_GLFW::draw_image(WindowControl::Picture *image)
{
    if(!image)return WindowDrawResult::WDR_PARAMETER_ERROR;
    if(s_imageShader.get_id()==0)
    {
